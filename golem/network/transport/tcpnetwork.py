...
class BroadcastProtocol(SafeProtocol):
    ...
    def __init__(self, session_factory, server, **_kwargs):
        logger.error('Init BroadcastProtocol')
    ...
    def dataReceivedHandshake(self, data: bytes) -> None:
        logger.error('handshake data received %sb', len(data))
        self.db.append_bytes(data)
        ...
        logger.error("Sucesfuly finished handshake with %s", self.transport.getPeer())
        return None
